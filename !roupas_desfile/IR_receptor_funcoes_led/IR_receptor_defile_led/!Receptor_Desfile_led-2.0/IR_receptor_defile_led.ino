...
#define RECV_PIN 6                                                                         // Arduino pino D11 conectado no Receptor IR
//#define LEDAMARELO 5
...
//int sensorIR = 6;                                                                           // Arduino pino D10 conectado no Receptor IR
...
float hexa;
    ...
    int ir_receptor(/*irrecv.decode(&results)*/)
    {
      if (irrecv.decode(&results))                                                          // se algum código for recebido
      {
        Serial.println(results.value, HEX);                                                 // imprime o RESULTADO = valor, no Monitor Serial
        hexa = results.value;                                                               // atribui o resultado HEX na variavel 'hexa'
        return hexa;                                                                        // retorna a variavel 'hexa' com o HEX para chamar a funcao de acendimento de leds
      }
    }
...
void loop() 
{
  ...
  if(hexa == valor){                                                                        //se valor recebido for igual ao 'valor' que é o HEX 0x11111
    piscarRapido();
